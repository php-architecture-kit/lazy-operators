...
namespace PhpArchitecture\LazyOperators\Foundation\Expression\Extension\List\Aggregate;
...
#[Group('List')]
#[Name('Product')]
#[Formula('f(values) = the product of values; computed via bcmath when available, native arithmetic otherwise')]
...
class ProductFunction implements NumberValue
{
    ...
    /**
     * Guard digits well past a PHP float's ~17 significant digits, so bcmul only ever
    ...
    public function __invoke(): int|float
    {
        ...
        $product = array_reduce(
            $values,
            static fn (string $carry, int|float $value): string => bcmul($carry, (string) $value, self::BC_SCALE),
            '1',
        );
